...
export async function POST(request: Request) {
  ...
  const { SLACK_BOT_TOKEN, SLACK_CHANNEL_VLAD_ID, SLACK_THREAD_BUGS_TS } = process.env;
  if (!SLACK_BOT_TOKEN || !SLACK_CHANNEL_VLAD_ID || !SLACK_THREAD_BUGS_TS) {
  ...
  if (typeof text !== "string" || !text.trim()) {
    return NextResponse.json({ error: "Missing bug description." }, { status: 400 });
  }
  const trimmed = text.trim().slice(0, MAX_LENGTH);

  const messageLines = [
    `*Bug report from* ${session.email}`,
  ...
  const slackResponse = await fetch("https://slack.com/api/chat.postMessage", {
    ...
    body: JSON.stringify({
      ...
      thread_ts: SLACK_THREAD_BUGS_TS,
