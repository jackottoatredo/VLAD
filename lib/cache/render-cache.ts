...
/**
 * Like {@link invalidateRenderCacheForRecording} but also returns the
 * intermediate R2 keys (background, overlay, composite, trim) that were
 * referenced from the cache before deletion. These artifacts live under
 * vlad/renders/, vlad/composites/, vlad/trims/ — they're not in any DB
 * column, so once the cache is gone the only way to find them is to
 * extract them now. Caller is expected to deleteManyFromR2 the result.
 */
export async function invalidateRenderCacheForRecordingWithKeys(
  userId: string,
  safeId: string,
): Promise<string[]> {
  const pattern = `cache:${CACHE_VERSION}:${userId}:${safeId}:*`;
  ...
  await new Promise<void>((resolve, reject) => {
    stream.on("data", (batch: string[]) => {
      for (const k of batch) cacheKeys.push(k);
    });
    stream.on("end", () => resolve());
    stream.on("error", reject);
  });
  ...
  if (cacheKeys.length > 0) {
    ...
    for (const k of cacheKeys) pipe.hgetall(k);
    const results = (await pipe.exec()) ?? [];
    for (const entry of results) {
      const data = entry?.[1] as Record<string, string> | null | undefined;
      if (!data) continue;
      for (const [field, val] of Object.entries(data)) {
        if (field.endsWith("_r2_key") && typeof val === "string" && val) {
          r2Keys.add(val);
        }
      }
    }
